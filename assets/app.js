...
async function submitWhitelist(e){e.preventDefault();const email=document.querySelector("#email").value.trim(),twitter=document.querySelector("#twitter").value.trim(),wallet=document.querySelector("#wallet").value.trim(),out=document.querySelector("#result");if(!email||!twitter||!wallet)return;
 const data={email,twitter_username:twitter,bnb_address:wallet};
 try{
